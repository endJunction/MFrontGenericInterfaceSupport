...
# MFRONT_CHECK_CXX_SOURCE_COMPILES(<code> <var> [FAIL_REGEX <fail-regex>])
...
#
#  MFRONT_CMAKE_REQUIRED_FLAGS = string of compile command line flags
#  MFRONT_CMAKE_REQUIRED_DEFINITIONS = list of macros to define (-DFOO=bar)
#  MFRONT_CMAKE_REQUIRED_INCLUDES = list of include directories
#  MFRONT_CMAKE_REQUIRED_LIBRARIES = list of libraries to link
#
# This macro is a copy of CheckCXXSourceCompiles.cmake
# Copyright 2005-2009 Kitware, Inc.
MACRO(MFRONT_CHECK_CXX_SOURCE_COMPILES SOURCE VAR)
    SET(_FAIL_REGEX)
    SET(_key)
    FOREACH(arg ${ARGN})
    ...
    SET(MACRO_CHECK_FUNCTION_DEFINITIONS
      "-D${VAR} ${MFRONT_CMAKE_REQUIRED_FLAGS}")
    IF(MFRONT_CMAKE_REQUIRED_LIBRARIES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_LIBRARIES
        "-DLINK_LIBRARIES:STRING=${MFRONT_CMAKE_REQUIRED_LIBRARIES}")
    ELSE(MFRONT_CMAKE_REQUIRED_LIBRARIES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_LIBRARIES)
    ENDIF(MFRONT_CMAKE_REQUIRED_LIBRARIES)
    IF(MFRONT_CMAKE_REQUIRED_INCLUDES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_INCLUDES
        "-DINCLUDE_DIRECTORIES:STRING=${MFRONT_CMAKE_REQUIRED_INCLUDES}")
    ELSE(MFRONT_CMAKE_REQUIRED_INCLUDES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_INCLUDES)
    ENDIF(MFRONT_CMAKE_REQUIRED_INCLUDES)
    ...
    TRY_COMPILE(${VAR}
      ...
      COMPILE_DEFINITIONS ${MFRONT_CMAKE_REQUIRED_DEFINITIONS}
    ...
    ELSE(${VAR})
      ...
      FILE(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
        ...
        "Source file was:\n${SOURCE}\n")
    ENDIF(${VAR})
ENDMACRO(MFRONT_CHECK_CXX_SOURCE_COMPILES)

MACRO (MFRONT_CHECK_CXX_COMPILER_FLAG _FLAG _RESULT)
   SET(SAFE_MFRONT_CMAKE_REQUIRED_DEFINITIONS "${MFRONT_CMAKE_REQUIRED_DEFINITIONS}")
   SET(MFRONT_CMAKE_REQUIRED_DEFINITIONS "${_FLAG}")
   MFRONT_CHECK_CXX_SOURCE_COMPILES("int main() { return 0;}" ${_RESULT}
   ...
   SET (MFRONT_CMAKE_REQUIRED_DEFINITIONS "${SAFE_MFRONT_CMAKE_REQUIRED_DEFINITIONS}")
ENDMACRO (MFRONT_CHECK_CXX_COMPILER_FLAG)

MACRO(tfel_enable_cxx_compiler_flag2 out flag var)
  if(MSVC)
	MFRONT_CHECK_CXX_COMPILER_FLAG("/${flag}" ${var})
  else(MSVC)
	MFRONT_CHECK_CXX_COMPILER_FLAG("-${flag}" ${var})
  endif(MSVC)
  IF(${var})
...
if(HAVE_NORETURN_ATTRIBUTE)
  ...
  set(COMPILER_CXXFLAGS "${COMPILER_CXXFLAGS} -DMFRONT_HAVE_NORETURN_ATTRIBUTE")
